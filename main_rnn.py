...
from PredictConfidenceNet import ConfidenceNet_rnn, ConfidenceNet_rnn_cls
...
def main():
    ...
    if args.cls:
        model = ConfidenceNet_rnn_cls(embed_dim=args.embed_dim, rnn_hidden_size=args.rnn_hidden_size, rnn_num_layers=args.rnn_num_layers, bias=args.bias, nonlinearity=args.nonlinearity, dropout=args.dropout, bidirectional=args.bidirectional, decoder_num_layers=args.decoder_num_layers, decoder_hidden_size=args.decoder_hidden_size, decoder_act=args.decoder_act)
        loss_fn = torch.nn.CrossEntropyLoss()

    else:
        model = ConfidenceNet_rnn(embed_dim=args.embed_dim, rnn_hidden_size=args.rnn_hidden_size, rnn_num_layers=args.rnn_num_layers, bias=args.bias, nonlinearity=args.nonlinearity, dropout=args.dropout, bidirectional=args.bidirectional, decoder_num_layers=args.decoder_num_layers, decoder_hidden_size=args.decoder_hidden_size, decoder_act=args.decoder_act)
    ...
    model_name = f"model_rnn_cls_{args.random_seed}.pth" if args.cls else f"model_rnn_reg_{args.random_seed}.pth"
    ...
    if args.cls:
        ...
        df.to_csv(f"{args.target_dir}/results_rnn_cls_{args.random_seed}.csv")
    else:
        df = pd.DataFrame({"train_loss": train_loss, "val_loss": val_loss, "train_r_square": train_r_square, "val_r_square": val_r_square})
        df.to_csv(f"{args.target_dir}/results_rnn_reg_{args.random_seed}.csv")
